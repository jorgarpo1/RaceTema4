...
```{r}
str(cran)
select(cran,r_arch,r_os,package,version,country)
```
...
```{r}
cran
cran2 <- cran %>% select(size,r_version,r_arch,r_os,package,version,country,ip_id)
```
...
top_unique_sorted<- pack_sum %>% 
  filter(unique > quantile(unique,probs = 0.99)) %>% arrange(desc(unique))
top_unique_sorted
```
...
```{r}
by_country <- cran %>% group_by(country)
by_country %>% summarise(total_descargas = n()) %>%
  filter(total_descargas >quantile(total_descargas,probs = 0.9))
  
ggplot(by_country,aes(x = country, y = total_descargas)) + geom_point()
